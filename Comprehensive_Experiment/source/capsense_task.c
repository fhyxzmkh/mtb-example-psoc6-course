...
cy_rslt_t send_capsense_data_to_server(uint32_t data);
...
static TickType_t last_send_time = 0;  // 记录最后一次发送的时间
...
static void process_touch(void)
{
    ...
    if((0u != button0_status) && (0u == button0_status_prev))
    {
        ...
        send_capsense_data_to_server(200);
    }
    ...
    if((0u != button1_status) && (0u == button1_status_prev))
    {
        ...
        send_capsense_data_to_server(300);
    }
    ...
    if((0u != slider_touched) && (abs(slider_pos - slider_pos_perv) > SLIDER_POSITION_HYSTERESIS))
    {
        ...
        if ((current_time - last_send_time) >= pdMS_TO_TICKS(50))
        {
            send_capsense_data_to_server(led_cmd_data.brightness);
...
cy_rslt_t send_capsense_data_to_server(uint32_t data)
{
    ...
    /* 发送Capsense数据到服务器 */
    printf("正在发送Capsense数据到服务器...\r\n");

    uint32_t bytes_sent = 0;
    char msg[10];
    sprintf(msg, "%c%d", 'C', data);
    
    result = cy_socket_sendto(client_handle, msg, strlen(msg), CY_SOCKET_FLAGS_NONE,
                                &server_addr, sizeof(cy_socket_sockaddr_t), &bytes_sent);
